 ...
 */
public class MesReservationVoyageorgForm extends   BaseForm {
    
    public MesReservationVoyageorgForm(Resources res) {
        ...
        ServiceReservVoyageorganise ser = new ServiceReservVoyageorganise();
       ServiceVoyageorganise se = new ServiceVoyageorganise();
					ArrayList<ReservationVoyageorganise> Tab = ser.getListmesvo(UserCo.userCo.getId());
                                        ArrayList<Voyageorganise> Tabb = se.getList5();
                                        Voyageorganise ee=new Voyageorganise();
                                         ReservationVoyageorganise et=new ReservationVoyageorganise();
					for (int i = 0; i < Tab.size(); i++) {
						/*addButton(res,Tab.get(i).getImage(),Tab.get(i).getNom_Hebergement(), false, 11, 9,Tab.get(i).getIdHebergement(),Tab.get(i).getIdAgence(),ee,Tab.get(i).getNomAgence(),Tab.get(i).getType_Hebergement(),Tab.get(i).getNombre_etoile(),Tab.get(i).getAdresse_Hebergement(),Tab.get(i).getNombre_chambre(),Tab.get(i).getPrix_single(),Tab.get(i).getPrix_double(),Tab.get(i).getTaux_demi(),Tab.get(i).getTaux_complet(),Tab.get(i).getTel(),Tab.get(i).getDescription());*/
                                               addButton(res,Tabb.get(i).getImage(), false, 11, 9,Tab.get(i).getIdVoyageorganise(),Tabb.get(i).getHotel(),ee,Tabb.get(i).getNom_agence(),Tabb.get(i).getOrigine(),Tab.get(i).getId_reservation(),Tab.get(i).getPrix(),Tabb.get(i).getDate_depart(),Tabb.get(i).getDate_retour(),Tab.get(i).getNbrplace(),et,Tab.get(i).getId_user());
  ...
  private void addButton(Resources res,String URLImage, boolean liked, int likeCount, int commentCount,int idHebergement, String title, Voyageorganise ee,String agg,String a, int b, int d, String f, String j, int h, ReservationVoyageorganise et, int gt) {
       ...
       image.addActionListener(e -> {
			...
			x = Dialog.show("Que voulez vous faire ?","","Annuler reservation","Reclamer");
                        ...
                        ServiceReservVoyageorganise bb =new ServiceReservVoyageorganise();
                           ArrayList<ReservationVoyageorganise> p= bb.getListmesvo(gt);
                           bb.supprimerreservvoyageorg(b);
                           new MesReservationVoyageorgForm(res).show();
